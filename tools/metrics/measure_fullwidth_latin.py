"""§4.6.2 round 50 — fullwidth Latin × autoSpaceDE.

Round 24 confirmed autoSpaceDE fires on halfwidth Latin (a-z, A-Z, 0-9)
adjacent to kana. Question: does it also fire on FULLWIDTH Latin
(Ａ-Ｚ, ０-９) which are typically rendered same-width as kana?

Probes (fs=12 MS Mincho):
  V1 halfwidth: 漢A → 漢 expected = 15.0 (boundary applied)
  V2 fullwidth letter: 漢Ａ → 漢 = ?
  V3 fullwidth digit: 漢１ → 漢 = ?
  V4 baseline halfwidth: 漢a → reference

Hypothesis: fullwidth Latin already same-width as CJK, so no boundary
spacing needed. Word may NOT fire autoSpaceDE on fullwidth.
"""
...
OUT_DIR = os.path.abspath(r"C:\Users\ryuji\oxi-1\tools\metrics\fullwidth_repro")
RESULT = os.path.abspath(r"C:\Users\ryuji\oxi-1\pipeline_data\fullwidth_latin.json")
...
def main():
    ...
    variants = [
        ("V1_half_a",   "漢a", "halfwidth letter (Round 24 ref: 漢=15)"),
        ("V2_full_A",   "漢Ａ", "fullwidth letter Ａ (U+FF21)"),
        ("V3_full_1",   "漢１", "fullwidth digit １ (U+FF11)"),
        ("V4_half_A",   "漢A", "halfwidth uppercase A"),
        ("V5_only_full", "Ａ漢", "fullwidth → kana"),
    ]

    print(f"Test: 漢 + Latin variants, fs=12 TNR + MSM\n")
    for label, probe, desc in variants:
        ...
        adv_str = " ".join(f"{c['ch']}={c['adv']}" for c in chars[:3])
        print(f"  {label}: {desc}")
        print(f"    {adv_str}")
