# -*- coding: utf-8 -*-
"""Read the fnbox sweep. Usage: _pb_fnbox_read.py word|oxi [--sweep lo hi step]
...
from _pb_fnbox_gen import OUT, MULTS, OWN, parse_sweep
...
cols = [(m, o) for m in MULTS for o in OWN]
print("%s   fn-boundary page-bottom box (line=auto multiplier x own ref)\n" % mode.upper())
print("  spacer |" + "".join("  m%d/o%d pg  final_y |" % (m, o) for m, o in cols))
for x in sw:
    cells = []
    for m, o in cols:
        docx = os.path.join(OUT, "s%05d_m%d_o%d.docx" % (x, m, o))
