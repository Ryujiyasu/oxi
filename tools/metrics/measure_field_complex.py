"""§5.AA round 60 — complex field (fldChar begin/separate/end).

Round 59 found fldSimple is layout-transparent. Round 60 verifies
complex field structure behaves identically.
...
Probes:
  V1 plain "Page 1" reference
  V2 complex field PAGE with cached '1'
  V3 'Page ' + complex PAGE field cached '1'
"""
...
OUT_DIR = os.path.abspath(r"C:\Users\ryuji\oxi-1\tools\metrics\field_complex_repro")
RESULT = os.path.abspath(r"C:\Users\ryuji\oxi-1\pipeline_data\field_complex.json")
...
def make_field_complex(instr, cached_text, sz_val):
    return (f'<w:r><w:rPr><w:sz w:val="{sz_val}"/></w:rPr>'
            '<w:fldChar w:fldCharType="begin"/></w:r>'
            f'<w:r><w:rPr><w:sz w:val="{sz_val}"/></w:rPr>'
            f'<w:instrText xml:space="preserve">{instr}</w:instrText></w:r>'
            f'<w:r><w:rPr><w:sz w:val="{sz_val}"/></w:rPr>'
            '<w:fldChar w:fldCharType="separate"/></w:r>'
            f'{make_run_text(cached_text, sz_val)}'
            f'<w:r><w:rPr><w:sz w:val="{sz_val}"/></w:rPr>'
...
def main():
    ...
    print(f"Complex field test, fs=12 TNR\n")

    variants = [
        ("V1_plain",
         make_para(make_run_text("Page 1", sz_val)),
         "plain 'Page 1' (control)"),
        ("V2_complex_PAGE",
         make_para(make_field_complex("PAGE", "1", sz_val)),
         "complex field PAGE cached '1'"),
        ("V3_text_plus_complex",
         make_para(make_run_text("Page ", sz_val) + make_field_complex("PAGE", "1", sz_val)),
         "'Page ' + complex PAGE field"),
    ]
