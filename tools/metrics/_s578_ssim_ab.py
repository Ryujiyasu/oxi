# -*- coding: utf-8 -*-
"""S578 — SSIM sentinel for the ・ (nakaguro) render compression.

Render every word_png doc base with DWrite A (OXI_S578_DISABLE=1) and B
(default ON). First compare PNG bytes A vs B; S578 only fires on no-type
...
def render(docx, disable, outdir):
    env = dict(os.environ)
    if disable:
        env["OXI_S578_DISABLE"] = "1"
    else:
        env.pop("OXI_S578_DISABLE", None)
