# -*- coding: utf-8 -*-
"""S545: the two surviving oikomi hypotheses.
  G: SHORT line (no overflow) with a 。）、 triple mid-line → the three
     advances. Is the trailing 、 natural (10.5, free candidate) or special?
  H: ed025c ③L2-shaped line: two 。）、 chains + isolated 、 + （×2,
     overflow ≈1.85 → does Word oikomi (=our budget model) or push (=ed025c)?
  I: H minus one chain (single 。）、 + isolated 、 + （×2, overflow 1.85).
jc=left, type=lines, MS Mincho sz=21, kern=2, compat14. ascii output.
"""
import os
import zipfile
...
DOCX = os.path.join(OUT, 's545_chained.docx')
...
# G: short line, no wrap: 国*5 。）、 国*5
TEXT_G = '国' * 5 + '。）、' + '国' * 5
IND_G = 0

# width model: each 。）、 chain = 5.25+5.25+10.5 = 21.0 (two halvings save 10.5)
# H: 国*8 。）、 国*6 （ 国*5 。）、 国*5 、 国*4 （ 国*9  = 44 chars
TEXT_H = '国'*8 + '。）、' + '国'*6 + '（' + '国'*5 + '。）、' + '国'*5 + '、' + '国'*4 + '（' + '国'*9
N_H = len(TEXT_H)
W_H = N_H * 10.5 - 4 * 5.25  # 4 halvings (two chains)
IND_H = int(round((CONTENT - (W_H - OVER)) * 20))
# I: single chain: 国*8 。）、 国*9 （ 国*8 、 国*6 （ 国*10 = 44? compute
TEXT_I = '国'*8 + '。）、' + '国'*9 + '（' + '国'*8 + '、' + '国'*6 + '（' + '国'*10
N_I = len(TEXT_I)
W_I = N_I * 10.5 - 2 * 5.25
IND_I = int(round((CONTENT - (W_I - OVER)) * 20))

paras = []
for text, ind in ((TEXT_G, IND_G), (TEXT_H, IND_H), (TEXT_I, IND_I)):
...
print('H: N=%d W=%.2f ind=%dtw | I: N=%d W=%.2f ind=%dtw' % (N_H, W_H, IND_H, N_I, W_I, IND_I))
...
try:
    ...
    try:
        for pi, p in enumerate(wdoc.Paragraphs):
            ...
            prev = None
            advs = []
            nmax = (N_H if pi == 1 else (N_I if pi == 2 else 13))
            for i in range(nmax):
            ...
            print('para %s L1=%d/%d small=%s' % ('GHI'[pi], n, nmax, small[:10]))
    finally:
        wdoc.Close(False)
finally:
    word.Quit()
