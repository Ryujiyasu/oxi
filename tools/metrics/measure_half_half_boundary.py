"""§4.6.2 round 54 — halfwidth kata × halfwidth Latin.

Final caveat from Round 51. Probe ｱa: halfwidth katakana (U+FF71) +
halfwidth Latin (a). Neither is fullwidth CJK; expect no boundary.

Probes:
  V1 ｱa: halfwidth kata → halfwidth Latin
  V2 aｱ: halfwidth Latin → halfwidth kata
  V3 ｱア (control): halfwidth kata → fullwidth kata (no boundary expected)
"""
...
OUT_DIR = os.path.abspath(r"C:\Users\ryuji\oxi-1\tools\metrics\half_half_repro")
RESULT = os.path.abspath(r"C:\Users\ryuji\oxi-1\pipeline_data\half_half.json")
...
def main():
    ...
    variants = [
        ("V1_kata_a",    "ｱa", "halfwidth kata + halfwidth Latin"),
        ("V2_a_kata",    "aｱ", "halfwidth Latin + halfwidth kata"),
        ("V3_kata_kata", "ｱア", "halfwidth + fullwidth katakana (control)"),
    ]

    print(f"Test: half × half boundary, fs=12 TNR + MSM\n")
