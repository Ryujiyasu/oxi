# -*- coding: utf-8 -*-
"""S576 — SSIM sentinel for the no-type-docGrid page-bottom ink-leniency.

Render every word_png doc base with DWrite A (OXI_S576_DISABLE=1) and B
(default ON). First compare PNG bytes A vs B; S576 only fires on no-type
...
def render(docx, disable, outdir):
    env = dict(os.environ)
    if disable:
        env["OXI_S576_DISABLE"] = "1"
    else:
        env.pop("OXI_S576_DISABLE", None)
