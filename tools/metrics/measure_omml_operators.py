"""Measure Word's handling of math operators / symbols in OMML runs.

Tests common math operators:
  - N-ary: ∑ ∏ ∐ ∫ ∬ ∭ ∮ ⋀ ⋁ ⋂ ⋃
  - Relations: = ≠ < > ≤ ≥ ≈ ≡ ∈ ∉ ⊂ ⊃ ⊆ ⊇
  - Binary ops: + - × ÷ · ∗ ± ∓
  - Calculus: ∂ ∇ ∞ lim
...
Hypothesis: operators already in math alphabet block are NOT substituted
(stay as input). Regular letters (A-Z Greek) ARE substituted (tested).

Output: tools/metrics/output/omml_operators_table.json
"""
...
TMP = Path("pipeline_data") / "_omml_ops_tmp.docx"
TMP.parent.mkdir(parents=True, exist_ok=True)
OUT = Path(__file__).with_name("output") / "omml_operators_table.json"
...
OPERATOR_GROUPS = [
    ...
    ("calculus", ['∂','∇','∞']),
    ("quantifiers", ['∀','∃','∄']),
    ("blackboard_bold", ['∅','ℕ','ℤ','ℚ','ℝ','ℂ']),
    ("arrows", ['→','⇒','↔','⇔','↦']),
]


def xml_escape(c):
    # Escape XML-significant chars inside <m:t> content
    if c == '<': return '&lt;'
    if c == '>': return '&gt;'
    if c == '&': return '&amp;'
    return c
...
def build_docx(chars):
    math_content = ''.join(f'<m:r><m:t>{xml_escape(c)}</m:t></m:r>' for c in chars)
    body = (
...
def main():
    ...
    if word is None:
        print("Word COM unavailable.")
        return

    results = {}
    try:
        for name, chars in OPERATOR_GROUPS:
            print(f"\n=== {name} ({len(chars)} chars) ===")
            r = measure(word, chars)
            if isinstance(r, dict) and "error" in r:
                print(f"  ERR: {r['error']}")
                continue
            if len(r) != len(chars):
                print(f"  MISMATCH: rendered={len(r)} input={len(chars)}")
                results[name] = {"mismatch": True, "rendered": r}
                continue
            table = []
            for i, c in enumerate(chars):
                sub = r[i]["cp"] != ord(c)
                entry = {"input_ch": c, "input_cp": ord(c),
                         "rendered_ch": r[i]["ch"], "rendered_cp": r[i]["cp"],
                         "substituted": sub}
                table.append(entry)
                mark = "→" if sub else " "
    ...
    print(f"  TOTAL: {total_subst}/{total_tested} substituted")
    print(f"Saved → {OUT}")
