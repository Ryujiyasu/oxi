...
def gdi_line_height(fm, pt_size, dpi):
    """GDI TEXTMETRIC simulation at given DPI."""
    upm = fm["units_per_em"]
...
def main():
    ...
    # ====== Try GDI hypothesis at various DPIs ======
    print("\n\n" + "=" * 100)
    print("GDI HYPOTHESIS: max(gdi(run_font, size, DPI), gdi(default_font, 11, DPI))")
    print("=" * 100)
    ...
    for dpi in [72, 96, 120, 144, 150, 192, 288]:
        gdi_default = gdi_line_height(calibri, 11.0, dpi)
        errors = []
        ...
        for c in com:
            ...
            gdi_val = gdi_line_height(fm[fm_key], size, dpi)
            predicted = max(gdi_val, gdi_default)
    ...
    print(f"{'Font':<12} {'Size':>5} {'COM':>7} {'GDI':>7} {'err':>7}")
    print("-" * 50)

    gdi_default = gdi_line_height(calibri, 11.0, best_dpi)
    ...
    for c in com:
        ...
        if not fm_key or fm_key not in fm:
            continue
        gdi_val = gdi_line_height(fm[fm_key], size, best_dpi)
        predicted = max(gdi_val, gdi_default)
        err = predicted - com_gap
    ...
    for dpi in [72, 96, 144, 150]:
        gdi_default_cambria = gdi_line_height(cambria, 11.0, dpi)
        errors = []
        ...
        for c in com:
            ...
            if not fm_key or fm_key not in fm:
                continue
            gdi_val = gdi_line_height(fm[fm_key], size, dpi)
            predicted = max(gdi_val, gdi_default_cambria)
            errors.append(abs(predicted - com_gap))

        avg_err = sum(errors) / len(errors)
        max_err = max(errors)
        within = sum(1 for e in errors if e <= 0.5)
        print(f"  DPI={dpi:>4}: default={gdi_default_cambria:.3f}pt, avg |err|={avg_err:.3f}pt, max={max_err:.3f}pt, {within}/{len(errors)} within 0.5pt")
    ...
    for dpi in [72, 96, 144, 150]:
        gdi_latin_default = gdi_line_height(cambria, 11.0, dpi)
        gdi_ea_default = gdi_line_height(msmincho, 11.0, dpi)
        errors = []
        ...
        for c in com:
            ...
            gdi_val = gdi_line_height(fm[fm_key], size, dpi)
            if fid in ea_fonts:
                predicted = max(gdi_val, gdi_ea_default)
            else:
                predicted = max(gdi_val, gdi_latin_default)
            errors.append(abs(predicted - com_gap))

        avg_err = sum(errors) / len(errors)
        max_err = max(errors)
        within = sum(1 for e in errors if e <= 0.5)
        print(f"  DPI={dpi:>4}: latin_def={gdi_latin_default:.3f}, ea_def={gdi_ea_default:.3f}, avg |err|={avg_err:.3f}pt, max={max_err:.3f}pt, {within}/{len(errors)} within 0.5pt")
    ...
    for dpi in [72, 96, 144, 150]:
        gdi_cambria = gdi_line_height(cambria, 11.0, dpi)
        gdi_msmincho = gdi_line_height(msmincho, 11.0, dpi)
        combined_default = max(gdi_cambria, gdi_msmincho)
        errors = []
        ...
        for c in com:
            ...
            if not fm_key or fm_key not in fm:
                continue
            gdi_val = gdi_line_height(fm[fm_key], size, dpi)
            predicted = max(gdi_val, combined_default)
            errors.append(abs(predicted - com_gap))

        avg_err = sum(errors) / len(errors)
        max_err = max(errors)
        within = sum(1 for e in errors if e <= 0.5)
        print(f"  DPI={dpi:>4}: cambria={gdi_cambria:.3f}, msmincho={gdi_msmincho:.3f}, combined={combined_default:.3f}, avg |err|={avg_err:.3f}pt, max={max_err:.3f}pt, {within}/{len(errors)} within 0.5pt")
    ...
    def gdi_hhea_line_height(fm_data, pt_size, dpi):
    ...
    for dpi in [72, 96, 144, 150]:
        hhea_cambria = gdi_hhea_line_height(cambria, 11.0, dpi)
        errors = []
        ...
        for c in com:
            ...
            if not fm_key or fm_key not in fm:
                continue
            hhea_val = gdi_hhea_line_height(fm[fm_key], size, dpi)
    ...
    # ====== Try max(win, hhea) GDI ======
    print("\n\n" + "=" * 100)
    print("HYPOTHESIS: max(gdi_win, gdi_hhea) per font + max with default")
    print("=" * 100)

    def gdi_max_line_height(fm_data, pt_size, dpi):
        return max(gdi_line_height(fm_data, pt_size, dpi),
                   gdi_hhea_line_height(fm_data, pt_size, dpi))

    for dpi in [72, 96, 144, 150]:
        default_h = gdi_max_line_height(cambria, 11.0, dpi)
        errors = []
        details = []
        ...
        for c in com:
            ...
            if not fm_key or fm_key not in fm:
                continue
            run_h = gdi_max_line_height(fm[fm_key], size, dpi)
    ...
    for dpi in [72, 96, 144, 150]:
        ...
        for c in com:
            ...
            if not fm_key or fm_key not in fm:
                continue
            run_win = gdi_line_height(fm[fm_key], size, dpi)
            run_hhea = gdi_hhea_line_height(fm[fm_key], size, dpi)
            run_h = max(run_win, run_hhea)

            def_win = gdi_line_height(cambria, size, dpi)
            def_hhea = gdi_hhea_line_height(cambria, size, dpi)
    ...
    for dpi in [72, 96, 144, 150]:
        errors = []
        details = []
        def11 = gdi_max_line_height(cambria, 11.0, dpi)
        ...
        for c in com:
            ...
            if not fm_key or fm_key not in fm:
                continue
            run_h = gdi_max_line_height(fm[fm_key], size, dpi)
            def_at_sz = gdi_max_line_height(cambria, size, dpi)
    ...
    # ====== Reverse-engineer: what default height per size matches COM? ======
    print("\n\n" + "=" * 100)
    print("REVERSE ENGINEERING: what 'default' value per font-size matches COM?")
