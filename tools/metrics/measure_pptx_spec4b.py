# -*- coding: utf-8 -*-
"""Measure Spec #4 wave-2 truth: COM structure + margins + PDF spans.

Adds to the wave-1 measurement:
  - TextFrame.MarginTop / MarginBottom / MarginLeft / MarginRight (pt),
    VerticalAnchor
  - per-run font name + size (COM)
  - PDF span FONT + SIZE per line (fitz), so the theme-default font can be
    identified and the first-baseline offset decomposed.

PDF lines carry {y0 (baseline), x0, x1, text, font, size} where font/size are
the FIRST non-empty span's, plus a full `spans` list.

Usage:
  python measure_pptx_spec4b.py <input.pptx> <out_dir>
"""
import json
import os
import sys
...
def _textframe_margins(sh):
    """Internal text-frame margins (pt) + vertical anchor."""
    try:
        tf = sh.TextFrame
        rec = {
...
def _runs(sh):
    """Per-paragraph run (text, font name, size)."""
    try:
        if not (sh.HasTextFrame and sh.TextFrame.HasText):
            return None
        tr = sh.TextFrame.TextRange
        ...
        for i in range(1, n + 1):
            pr = tr.Paragraphs(i)
            try:
                nrun = int(pr.Runs().Count)
            except Exception:
                nrun = 0
            runs = []
            for k in range(1, nrun + 1):
                rr = pr.Runs(k)
                rec = {"text": str(rr.Text)}
                try:
                    rec["font"] = str(rr.Font.Name)
                except Exception:
                    rec["font"] = None
                try:
                    rec["size"] = round(float(rr.Font.Size), 3)
                except Exception:
                    rec["size"] = None
                runs.append(rec)
            out.append(runs)
...
def measure_pdf(pdf_path):
    """Per page: {page, width, height, lines:[{y0,x0,x1,text,font,size,spans}]}."""
...
def main():
    ...
    with open(os.path.join(out_dir, "spec4b_truth.json"), "w", encoding="utf-8") as f:
    ...
    print("wrote", os.path.join(out_dir, "spec4b_truth.json"))
