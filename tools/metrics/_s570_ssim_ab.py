...
ON (default) and OFF (OXI_S570_DISABLE=1) — then SSIM each page vs the cached
...
def render(disable, outroot):
    env = dict(os.environ)
    if disable:
        env["OXI_S570_DISABLE"] = "1"
    else:
        env.pop("OXI_S570_DISABLE", None)
    oxi = {}
...
with tempfile.TemporaryDirectory(prefix="s559_off_") as off_dir, \
    ...
    print("rendering OFF (OXI_S570_DISABLE=1)...")
