"""Verify Mech 1 (FINAL RULE Type A/B/C) independence from
characterSpacingControl.

Earlier session 51 measure_yakumono_setting_contrast.py (4-font
validation) found doNotCompress and compressPunctuation gave IDENTICAL
results for `漢」（漢` (B→A pair). Conclusion: characterSpacingControl
is NOT a Mech 1 gate.

But session 51 mech3_trigger bisection found compressPunctuation IS the
gate for Mech 2/Mech 3 (CJK-flanked compression).

Reconciliation hypothesis: Mech 1 is gated by kern only;
Mech 2/Mech 3 are gated by kern + compressPunctuation.

This verifies that hypothesis with controlled tests.

Variants:
  V_M1_dnc:    doNotCompress + `漢」（漢`     → expect Mech 1 fires
  V_M1_cp:     compressPunctuation + `漢」（漢`  → expect Mech 1 fires (same)
  V_M3_dnc:    doNotCompress + `（地方卸売市場）に係る` → expect 0
  V_M3_cp:     compressPunctuation + `（地方卸売市場）に係る` → expect Mech 3 fires

If V_M1_dnc fires Mech 1 same as V_M1_cp, hypothesis confirmed.
If V_M1_dnc shows 0 compression, characterSpacingControl gates Mech 1
too (refines/refutes earlier session 51 4-font finding).
"""
import os
import time
import json
...
OUT_DIR = os.path.abspath("pipeline_data/mech1_vs_charSpacing_docs")
RESULT_PATH = os.path.abspath(
    "pipeline_data/mech1_vs_charSpacing_2026-05-02.json")
...
def write_docx(path, charSpacing, text):
    tmp = tempfile.mkdtemp(prefix="m1cs_")
    try:
...
VARIANTS = [
    # (label, charSpacing, text, mech tested)
    ("V_M1_dnc",   "doNotCompress",       "漢」（漢"),
    ("V_M1_cp",    "compressPunctuation", "漢」（漢"),
    ("V_M3_dnc",   "doNotCompress",
     "卸売市場法第６条第１項（第14条において準用する同法第６条第１項）の規定により、中央卸売市場（地方卸売市場）に係る認定事項の変更について"),
    ("V_M3_cp",    "compressPunctuation",
     "卸売市場法第６条第１項（第14条において準用する同法第６条第１項）の規定により、中央卸売市場（地方卸売市場）に係る認定事項の変更について"),
]
...
def main():
    ...
    results = {}
    for label, cs, text in VARIANTS:
        path = os.path.join(OUT_DIR, f"{label}.docx")
        write_docx(path, cs, text)
        ...
        try:
            ...
            mech_other = [a for a in yak_compressed
                           if a["rule_match"] == "none"]
            results[label] = {
                "charSpacing": cs, "text": text,
                "n_compressed": len(yak_compressed),
                "n_mech1": len(mech1_hits),
                "n_mech_other": len(mech_other),
                "advs": advs,
            }
            print(f"\n[{label}] charSpacing={cs} text={text[:40]!r}",
                  flush=True)
            print(f"  total_compressed={len(yak_compressed)} "
                  f"M1_hits={len(mech1_hits)} other={len(mech_other)}",
                  flush=True)
