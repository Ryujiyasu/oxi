"""§5.X round 70 — w:pBdr paragraph borders.

ECMA-376 §17.3.1.24 — `<w:pBdr>` element with sub-elements:
  <w:top w:val="single" w:sz="12" w:space="..." w:color="..."/>
  <w:left w:val="single" w:sz="12" w:space="..." w:color="..."/>
  <w:bottom .../>
  <w:right .../>
  <w:between .../>
  <w:bar .../>

w:sz in 1/8 of point. w:space in pt.

Probes:
  V1 plain (no border): reference y
  V2 top border 12 (=1.5pt) space 4
  V3 all 4 sides
  V4 box border with large space
"""
...
OUT_DIR = os.path.abspath(r"C:\Users\ryuji\oxi-1\tools\metrics\para_borders_repro")
RESULT = os.path.abspath(r"C:\Users\ryuji\oxi-1\pipeline_data\para_borders.json")
os.makedirs(OUT_DIR, exist_ok=True)

CJK_FONT = "ＭＳ 明朝"


def make_doc_xml(border_xml, page_w_tw, margin_tw):
...
def main():
    ...
    print(f"Paragraph border test, fs=12 MS Mincho\n")

    variants = [
        ("V1_no_border", "", "no border (control)"),
        ("V2_top_only",
         '<w:pBdr><w:top w:val="single" w:sz="12" w:space="4" w:color="000000"/></w:pBdr>',
         "top border 1.5pt, space 4pt"),
        ("V3_all_sides",
         '<w:pBdr>'
        ...
        ("V4_thick_box",
         '<w:pBdr>'
         '<w:top w:val="single" w:sz="48" w:space="12" w:color="000000"/>'
         '<w:left w:val="single" w:sz="48" w:space="12" w:color="000000"/>'
         '<w:bottom w:val="single" w:sz="48" w:space="12" w:color="000000"/>'
         '<w:right w:val="single" w:sz="48" w:space="12" w:color="000000"/>'
         '</w:pBdr>',
         "thick (6pt) all sides, space 12pt"),
    ]

    for label, border, desc in variants:
        doc_xml = make_doc_xml(border, page_w_tw, margin_tw)
        try:
