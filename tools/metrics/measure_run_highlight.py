"""§5.FF round 72 — run-level highlight + shd.

ECMA-376 §17.3.2.15 — `<w:highlight w:val="yellow|cyan|...|none">` predefined highlight colors.
ECMA-376 §17.3.2.32 — `<w:shd>` in rPr (run-level shading, similar to pPr but per-run).

Probes:
  V1 plain "abcd"
  V2 highlight=yellow
  V3 shd in rPr (run-level)
  V4 highlight + shd combined
"""
...
OUT_DIR = os.path.abspath(r"C:\Users\ryuji\oxi-1\tools\metrics\run_highlight_repro")
RESULT = os.path.abspath(r"C:\Users\ryuji\oxi-1\pipeline_data\run_highlight.json")
...
def make_run(text, sz_val, rpr_extra=""):
    return ('<w:r><w:rPr>'
...
def main():
    ...
    print(f"Run-level highlight/shading test, fs=12 TNR\n")

    variants = [
        ("V1_plain", make_run("abcd", sz_val), "plain"),
        ("V2_highlight",
         make_run("abcd", sz_val, '<w:highlight w:val="yellow"/>'),
         "highlight=yellow"),
        ("V3_run_shd",
         make_run("abcd", sz_val, '<w:shd w:val="clear" w:color="auto" w:fill="FFCCCC"/>'),
         "run-level shd fill=FFCCCC"),
        ("V4_combined",
         make_run("abcd", sz_val, '<w:highlight w:val="cyan"/><w:shd w:val="clear" w:color="auto" w:fill="FFCCCC"/>'),
         "highlight + shd combined"),
    ]
    ...
    for label, body_xml, desc in variants:
        ...
        print(f"  {label}: {desc}")
        print(f"    n={entry.get('n_chars')} | {chars_str}")
