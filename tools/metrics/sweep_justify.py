"""Test if justify (jc=both) triggers '（' compression.

Fixed: cSC+compat15, MS Gothic 12pt, d77a-like text.
Vary: jc attribute (both/left/none).
"""
...
def make_docx(out, jc_val=None):
    jc = f'<w:jc w:val="{jc_val}"/>' if jc_val else ''
    doc = f'''<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">
<w:body>
<w:p><w:pPr>{jc}<w:rPr><w:rFonts w:ascii="ＭＳ ゴシック" w:eastAsia="ＭＳ ゴシック" w:hAnsi="ＭＳ ゴシック"/><w:sz w:val="24"/></w:rPr></w:pPr><w:r><w:rPr><w:rFonts w:ascii="ＭＳ ゴシック" w:eastAsia="ＭＳ ゴシック" w:hAnsi="ＭＳ ゴシック"/><w:sz w:val="24"/></w:rPr><w:t xml:space="preserve">{TEXT}</w:t></w:r></w:p>
...
for jc in [None, "left", "both", "distribute"]:
    out = os.path.join(TMP, f"jc_{jc}.docx")
    make_docx(out, jc)
    try:
        yaks = measure_all_yak(out)
        print(f"\njc={jc}:")
        for ch, x, y, adv in yaks[:8]:
            print(f"  '{ch}' x={x:>7} y={y:>6} advance={adv}")
    except Exception as e:
        print(f"jc={jc}: ERROR {e}")
