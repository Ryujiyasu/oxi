"""S478 gate: toggle OXI_S469_DISABLE (anchor_flow_offset natural-Y).
...
TMP=os.path.join(tempfile.gettempdir(), r"s478gate"); os.makedirs(TMP,exist_ok=True)
def render(dp,pfx,on):
    env=dict(os.environ)
    if on: env.pop("OXI_S478_DISABLE",None)
    else: env["OXI_S478_DISABLE"]="1"
