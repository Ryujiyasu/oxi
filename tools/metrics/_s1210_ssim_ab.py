# -*- coding: utf-8 -*-
"""SSIM A/B sentinel with a BASE env set held in BOTH arms.

`ssim_ab.py` compares "flags on" against "everything off", which cannot measure a
flag that only makes sense INSIDE a bundle (S1210 flips a1d6e4ef's sign depending
on whether the derived-cell bundle is on). BASE below is set in both arms; the
argument flags are added to the B arm only, so net(B-A) is the FLAG's own effect
and a POSITIVE net means the flag helps.
...
def render(docx,a,outdir):
    ...
    env.update(BASE)
    for k,v in AENV:
        if a: env.pop(k,None)          # A arm = base only
        else: env[k]=v                 # B arm = base + the flag under test
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    print(f"A=base({','.join(sorted(BASE))}) vs B=base+{','.join(k+'='+v for k,v in AENV)}"
          f" | checked {checked}; {len(changed)} changed bytes")
