# -*- coding: utf-8 -*-
"""S559 — SSIM A/B sentinel (FIXED: bypass render_with_oxi's skip-if-cached).
Render each target doc with the DWrite binary directly to per-mode temp dirs —
ON (default) and OFF (OXI_S559_DISABLE=1) — then SSIM each page vs the cached
...
with tempfile.TemporaryDirectory(prefix="s559_off_") as off_dir, \
    ...
    print("rendering OFF (OXI_S559_DISABLE=1)...")
