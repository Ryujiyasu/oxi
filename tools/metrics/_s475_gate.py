...
OFF = no env, ON = OXI_S475_BREAKBUDGET=1 (+ PAIR/SOLO from outer env).
...
TMP = os.path.join(tempfile.gettempdir(), r"s475gate")
os.makedirs(TMP, exist_ok=True)
def render(docx_path, prefix, on):
    env = dict(os.environ)
    if on:
        env["OXI_S475_BREAKBUDGET"] = "1"
    else:
        env.pop("OXI_S475_BREAKBUDGET", None)
...
def main():
    ...
    txt = "\n".join(out)
    open(os.path.join(REPO, "tools", "metrics", "_s475_gate.out"), "w", encoding="utf-8").write(txt)
