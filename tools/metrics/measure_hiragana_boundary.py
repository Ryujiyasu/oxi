"""§4.6.2 round 52 — hiragana × autoSpaceDE.

Round 24+ tests used 漢 (kanji ideograph U+6F22). ECMA spec says
"east Asian text" which includes hiragana, katakana, ideograph.
Verify boundary fires on hiragana ↔ halfwidth Latin too.

Probes (fs=12 MS Mincho):
  V1 漢a (kanji): reference, 漢=15.0 boundary fires
  V2 あa (hiragana U+3042): あ=? boundary expected
  V3 アa (fullwidth katakana U+30A2): ア=? boundary expected
  V4 a漢: a=7.5 with RSB asymmetry boundary
  V5 aあ: a=? expected same as V4
  V6 aア: a=? expected same as V4
"""
...
OUT_DIR = os.path.abspath(r"C:\Users\ryuji\oxi-1\tools\metrics\hiragana_repro")
RESULT = os.path.abspath(r"C:\Users\ryuji\oxi-1\pipeline_data\hiragana_boundary.json")
...
def measure_one(path):
    ...
    try:
        ...
        for i in range(len(xs)):
            ...
            char_info.append({"ch": repr(xs[i][0]), "ord": ord(xs[i][0][0]) if xs[i][0] else None, "adv": adv})
...
def main():
    ...
    variants = [
        ("V1_kanji_a",     "漢a",  "kanji 漢 (U+6F22) + a"),
        ("V2_hira_a",      "あa",  "hiragana あ (U+3042) + a"),
        ("V3_kata_a",      "アa",  "fullwidth katakana ア (U+30A2) + a"),
        ("V4_a_kanji",     "a漢",  "a + kanji"),
        ("V5_a_hira",      "aあ",  "a + hiragana"),
        ("V6_a_kata",      "aア",  "a + fullwidth katakana"),
    ]

    print(f"Test: kanji vs hiragana vs katakana boundary, fs=12 TNR + MSM\n")
