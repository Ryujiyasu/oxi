...
2. Compute line height at various DPIs with Word rounding
...
def compute_word_line_height(fm, pt_size, dpi=96):
    """Simulate Windows Word font metrics calculation."""
    upm = fm["units_per_em"]
    # lfHeight = -round(pt_size * dpi / 72) → em height in pixels
    ppem = round(pt_size * dpi / 72)

    # Word uses win metrics (unless USE_TYPO_METRICS is set)
    win_asc = fm["win_ascent"]
    win_desc = fm["win_descent"]

    # fontAscent = ceil(winAscent * ppem / UPM)
...
def compute_word_typo_line_height(fm, pt_size, dpi=96):
...
def main():
    ...
    print("=" * 120)
    print("HYPOTHESIS: Word font metrics-based line height at various DPIs")
    print("=" * 120)

    for dpi in [72, 96, 144, 150, 288]:
        print(f"\n--- DPI = {dpi} ---")
        print(f"{'Font':<12} {'Size':>5} {'Measured':>8} {'Word_win':>8} {'err':>7} {'Word_typo':>9} {'err':>7}")
        print("-" * 65)
        ...
        for entry in isolated:
            ...
            word_win = compute_word_line_height(fm, size, dpi)
            word_typo = compute_word_typo_line_height(fm, size, dpi)
            err_win = sng - word_win
            err_typo = sng - word_typo

            print(f"  {fid:<12} {size:>5} {sng:>8.3f} {word_win:>8.3f} {err_win:>+7.3f} {word_typo:>9.3f} {err_typo:>+7.3f}")
    ...
    # Hypothesis: max(run_font_height, default_font_height)
    ...
    for dpi in [72, 96, 144, 150]:
        default_height = compute_word_line_height(calibri, 11.0, dpi)
        default_height_typo = compute_word_typo_line_height(calibri, 11.0, dpi)
        ...
        for entry in isolated:
            ...
            max_win = max(word_win, default_height)
            max_typo = max(word_typo, default_height_typo)
    ...
    # Hypothesis: floating point (no Word rounding), max with default
    print("\n\n" + "=" * 120)
