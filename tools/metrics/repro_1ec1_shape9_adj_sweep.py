# -*- coding: utf-8 -*-
"""V_Q: Sweep adj 0..50000 in Shape 9 (cy=238.50pt) to map roundRect inset function."""
...
OUT_DIR = os.path.abspath("pipeline_data/1ec1_shape9_adj_sweep")
...
# Sweep adj
ADJ_VALUES = [0, 1000, 2000, 4015, 8000, 15000, 25000, 35000, 50000]
...
def main():
    ...
    print(f"Target: 1ec1 Shape 9 (adj=4015) BOX[5] advance 55.32pt")
    print(f"Predicted shape_left + lIns + ind = 36.275 + 2.835 + 5.25 + 0.5(ln) - 1.08(eff) = 43.78pt")
    print(f"So inset_observed = advance - 43.78pt\n")
    print(f"adj   inset_synth_observed (advance pt)")
    results = []
    try:
        for adj in ADJ_VALUES:
            print(f"\n=== adj={adj} ===")
            docx = os.path.abspath(os.path.join(OUT_DIR, f"adj_{adj}.docx"))
            pdf = os.path.abspath(os.path.join(OUT_DIR, f"adj_{adj}.pdf"))
            for f in (docx, pdf):
                try: os.remove(f)
                except: pass
            write_docx(docx, adj=adj)
            ok = render_pdf(word, docx, pdf)
            if not ok:
                results.append({"adj": adj, "error": "render failed"})
                continue
            m = measure_box(pdf)
            if m:
                ex_a = m["search_x0"] - LEFT_MARGIN_PT
                inset = m["search_x0"] - 43.78
                print(f"  search={m['search_x0']:.2f}pt | excess_adv={ex_a:.2f}pt | implied_inset={inset:.2f}pt")
                results.append({"adj": adj, "search_x0": m["search_x0"], "implied_inset": inset})
            else:
                results.append({"adj": adj, "error": "no glyph"})
    finally:
        try: word.Quit()
        except: pass
