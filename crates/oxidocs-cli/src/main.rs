...
fn main() {
    ...
    match args.get(1).map(|s| s.as_str()) {
        ...
        Some("vba-analyze") => {
            ...
            if let Err(error) = vba::analyze_file(input) {
        ...
        Some("vba-inventory") => {
            let input = args
                .get(2)
                .expect("Usage: oxi vba-inventory <file-or-directory>");
            if let Err(error) = vba::inventory(input) {
                eprintln!("VBA inventory failed: {error}");
                std::process::exit(2);
            }
        }
