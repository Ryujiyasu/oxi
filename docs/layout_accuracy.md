...
| 2026-07-17 | per-page **0.9378** · per-doc **0.9591** | **Phase 3** · Phase 1 87/87 · EN 6/6 | The English-corpus program (S802-S897): all 6 real English documents reached **100% paragraph-page pagination** (2026-07-14) and their per-doc SSIM (**0.853**) now leads LibreOffice (0.789) on the same ground truth — via footnote/footer reservation-stack derivations, metric tables for missing fonts (Aptos, Arial Narrow, Segoe UI, Bookman, SimSun aliases), the default table-style cell margin, floating-table flow rules, and contextualSpacing's layered gap model. A 50-document wild-English benchmark opened (frozen selection from the public docx-corpus): Oxi 0.824 vs LibreOffice 0.853, 41/50 at pagination match — the current work queue. Every document in both frozen corpora now renders ≥ 0.80 vs Word (the v0.8 floor) |
